...
#define TG3_REVISION		"d"
#define DRV_MODULE_VERSION	\
	__stringify(TG3_MAJ_NUM) "." __stringify(TG3_MIN_NUM)\
	TG3_REVISION
#define DRV_MODULE_RELDATE	"August 07, 2013"
			  ...
			  struct ptp_clock_request *rq, int on)
{
	...
	case PTP_CLK_REQ_PEROUT:
		...
		if (on) {
			...
			sec = rq->perout.start.sec * 1000000000ULL +
			...
			tw32(TG3_EAV_WATCHDOG0_LSB, (sec & 0xffffffff));
			tw32(TG3_EAV_WATCHDOG0_MSB,
			     TG3_EAV_WATCHDOG0_EN |
			     ((sec >> 32) & TG3_EAV_WATCHDOG_MSB_MASK));
...
static int tg3_chip_reset(struct tg3 *tp)
{
	...
	/* Set the clock policy to the highest frequency to avoid timeouts */
	if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_5718) {
		u32 oride = tr32(TG3_CPMU_CLCK_ORIDE);
		oride &= ~CPMU_CLCK_ORIDE_MAC_CLK_MASK;
		tw32(TG3_CPMU_CLCK_ORIDE, oride);

		tw32(TG3_CPMU_CLCK_ORIDE_ENABLE, TG3_CPMU_MAC_ORIDE_ENABLE);
	}
...
static int tg3_request_irq(struct tg3 *tp, int irq_num)
{
	...
	if (tg3_flag(tp, USING_MSI) || tg3_flag(tp, USING_MSIX)) {
		...
		flags = IRQF_SHARED|IRQF_TRIGGER_LOW;
#else
		flags = IRQF_SHARED;
#endif
	}
