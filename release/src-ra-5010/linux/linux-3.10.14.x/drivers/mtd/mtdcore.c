...
#ifdef CONFIG_MTK_MTD_NAND	//ASUS_EXT
...
static int __sync_factory2_eeprom(unsigned char *buf)
{
	...
	err1 = mtd_write(mtd, 0, mtd->erasesize, &wrlen, buf);
	if (err1 || wrlen != mtd->erasesize) {
	      ...
	      const u_char *buf)
{
	...
	*retlen = 0;
	...
	if (!strcmp(mtd->name, FACTORY_MTD_NAME)) {
		int alloc_type = 0;
		unsigned char *p = NULL;

		//printk("%s: name-%s, to-0x%llx, len-0x%llx\n", __func__, mtd->name, to, len);

		p = kmalloc(mtd->erasesize, GFP_KERNEL);
		if (!p) {
			if ((p = vmalloc(mtd->erasesize)) != NULL)
				alloc_type = 1;
			else {
