...
static DEFINE_MUTEX(bcmchar_mutex);
...
static long bcm_char_unlocked_ioctl(struct file *filp, 
					UINT cmd, ULONG arg)
{
	struct inode *inode = filp->f_mapping->host;
	long ret;
	mutex_lock(&bcmchar_mutex);
	ret = bcm_char_ioctl(inode, filp, cmd, arg);
	mutex_unlock(&bcmchar_mutex);
	return ret;
}
#endif
