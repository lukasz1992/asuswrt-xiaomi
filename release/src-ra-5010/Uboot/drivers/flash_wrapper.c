...
#define EEPROM_SET_HEADER_OFFSET	(CFG_BLOCKSIZE - 2*1024)
#define MAX_EEPROM_SET_LENGTH		EEPROM_SET_HEADER_OFFSET
#else
#define MAX_EEPROM_SET_LENGTH		65535
#endif	/* ON_BOARD_NAND_FLASH_COMPONENT */
#else
#define MAX_EEPROM_SET_LENGTH		65535
#endif	/* MT7621_ASIC_BOARD*/
#endif
...
int ra_flash_init_layout(void)
{
	...
	unsigned char factory[CFG_BLOCKSIZE], factory2[CFG_BLOCKSIZE];
	int r;
	int factory_r = 0, factory2_r = 0;

	if ((r = ra_flash_read(factory, CFG_FACTORY_ADDR, CFG_BLOCKSIZE)) != 0) {
	...
	if ((r = ra_flash_read(factory2, CFG_FACTORY2_ADDR, CFG_BLOCKSIZE)) != 0) {
	...
	if (factory_r == 1 && factory2_r == -3) {
		...
		r = ra_flash_erase_write(factory, CFG_FACTORY2_ADDR, CFG_BLOCKSIZE, 0);
		if (r)
			printf("Write EEPROM set to 0x%x fail. (r = %d)\n", CFG_FACTORY2_ADDR, r);
	}
	else if (factory_r == -3 && factory2_r == 1) {
		printf("Copy Factory2's EEPROM to Factory's one\n");
		r = ra_flash_erase_write(factory2, CFG_FACTORY_ADDR, CFG_BLOCKSIZE, 0);
		if (r)
			printf("Write EEPROM set to 0x%x fail. (r = %d)\n", CFG_FACTORY_ADDR, r);
	}
