...
printf("%s:%s:%d is_filter = %d\n", __FILE__, __FUNCTION__, __LINE__, is_filter);
...
printf("%s:%s:%d\n", __FILE__, __FUNCTION__, __LINE__);			
			reg_read(REG_ESW_TABLE_STATUS0, &value);
printf("%s:%s:%d REG_ESW_TABLE_STATUS0 = %d\n", __FILE__, __FUNCTION__, __LINE__, value);						
			if (value & 0x1) { //search_rdy
				if ((value & 0x70) == 0) {
					return;
				}
				...
				reg_read(REG_ESW_TABLE_STATUS2, &mac);
printf("%s:%s:%d REG_ESW_TABLE_STATUS2 = %d\n", __FILE__, __FUNCTION__, __LINE__, mac);								
				sprintf(v[0], "%08x", mac);
				reg_read(REG_ESW_TABLE_STATUS1, &mac);
printf("%s:%s:%d REG_ESW_TABLE_STATUS1 = %d\n", __FILE__, __FUNCTION__, __LINE__, mac);												
				sprintf(v[0]+8, "%04x", (mac & 0xffff));
				argv[2] = v[0];
				argv[3] = v[1];
printf("%s:%s:%d argv[2] = %d  argv[3]\n", __FILE__, __FUNCTION__, __LINE__, argv[2], argv[3]);																
				table_del(4, argv);
				if (value & 0x2) {
					return;
				}
