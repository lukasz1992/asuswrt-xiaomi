...
#include "torrent.h"
#include "utils.h"
#include "socket-server.h" //2013.11.27 magic added
struct tr_tier;
...
onUpkeepTimer (evutil_socket_t foo UNUSED, short bar UNUSED, void * vannouncer)
{
    ...
    flushCloseMessages (announcer);
    //2013.11.27 magic added{
     if (!is_closing){
                int memory_free = getFreeMem(); //2011.12.20 magic added
                int min_memory = getMinMem()*3; //2013.06.24 magic added
                //fprintf(stderr,"\nmemory_free=%d\n",memory_free);
                //fprintf(stderr,"\ntr_sessionGetPeerLimit( announcer->session )=%d\n",tr_sessionGetPeerLimit( announcer->session ));
                    if(memory_free<min_memory && tr_sessionGetPeerLimit( announcer->session )>80){
                            //fprintf(stderr,"\n!!!!!!!test\n");
                            tr_sessionSetPeerLimit( announcer->session, 80 );
                            tr_sessionLimitSpeed( announcer->session, TR_UP, TRUE );
                            if(up_rate_tmp>80){
                                    tr_sessionSetSpeedLimit_KBps( announcer->session, TR_UP, 40 );
                            }
                            else{
                                    tr_sessionSetSpeedLimit_KBps( announcer->session, TR_UP, (int)(up_rate_tmp/2) );
                            }
                    }else if(memory_free<min_memory && tr_sessionGetPeerLimit( announcer->session )==80){
                            //fprintf(stderr,"\n!!!!!!!test1\n");
                            tr_sessionLimitSpeed( announcer->session, TR_UP, TRUE );
                            if(up_rate_tmp>80){
                                    tr_sessionSetSpeedLimit_KBps( announcer->session, TR_UP, 40 );
                    ...
                    else if(tr_sessionGetPeerLimit( announcer->session )!=max_peer_tmp){
                            //fprintf(stderr,"\nmax_peer_tmp=%d\n",max_peer_tmp);
                            //fprintf(stderr,"\nup_rate_tmp=%d\n",up_rate_tmp);
                            //fprintf(stderr,"\nup_limit_enable=%d\n",up_limit_enable);
                            if(up_limit_enable==0){
                            tr_sessionLimitSpeed( announcer->session, TR_UP, FALSE );
                            }
                            else if(up_limit_enable==1){
                                    tr_sessionLimitSpeed( announcer->session, TR_UP, TRUE );
                            }
                            tr_sessionSetPeerLimit( announcer->session, max_peer_tmp );
                            tr_sessionSetSpeedLimit_KBps( announcer->session, TR_UP, up_rate_tmp );
                    }
            }
    //2013.11.27 magic added}
