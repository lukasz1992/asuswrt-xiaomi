   ...
   /*    FILE *fp;
      ...
      /* if(nvram_val==1){
           ...
           if (!tr_ssha1_matches (server->username, buf_username)|| !tr_ssha1_matches (server->password,buf_dec))
           {
               ...
               tr_rpcSetPassword (server,buf_dec);

           }

       }else {
           if (!tr_ssha1_matches (server->username, buf_username)|| !tr_ssha1_matches (server->password,buf_password))
           {
               tr_rpcSetUsername (server,buf_username);
               tr_rpcSetPassword (server,buf_password);
      ...
      if(access("/etc/shadow",0)==0){
          shadow_cp=compare_passwd_in_shadow(user,pass);
          us_na=0;
          no_encrypt=0;
      }else {
          nvram_val=nvram_encrypt_support();
#ifdef DM_MIPSBIG
          buf_username=(char *)malloc(32);
          buf_password=(char *)malloc(128);
          memset(buf_username,0,32);
          memset(buf_password,0,128);
          if(access("/userfs/bin/tcapi",0) == 0){
          buf_username=tcapi_get_by_popen("Account_Entry0","username");
          buf_password=tcapi_get_by_popen("Account_Entry0","web_passwd");
          }else{
              buf_username=nvram_get_by_popen("http_username");
              buf_password=nvram_get_by_popen("http_passwd");
          }
#else
          buf_username=(char *)malloc(32);
          buf_password=(char *)malloc(128);
          ...
          if(nvram_val==1){
             ...
             if((pw_de==1)&& !strcmp(user,buf_username)){
              ...
              if(!strcmp(user,buf_username) && !strcmp(pass,buf_password)){
                no_encrypt=1;
              }
              shadow_cp=0;
              us_na=0;
     ...
     /* else if (server->isPasswordEnabled
                 && (!pass || !user || !tr_ssha1_matches (server->username, user)
                                     || !tr_ssha1_matches (server->password,
                                                           pass)))*/
          else if((server->isPasswordEnabled
                   &&(shadow_cp==0)&&(us_na==0)&&(no_encrypt==0)) )
      {
          evhttp_add_header (req->output_headers,
                             "WWW-Authenticate",
...
tr_rpcSetUsername (tr_rpc_server * server, const char * username)
{
  ...
  if (*username != '{')
    server->username = tr_ssha1 (username);
  else
