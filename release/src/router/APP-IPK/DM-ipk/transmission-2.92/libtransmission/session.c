...
#include "socket-server.h" // added by Yan 2013.11.27 magic
#include "session.h"
#include "stats.h"
#include "torrent.h"
...
enum
{
  ...
  //DEFAULT_CACHE_SIZE_MB = 4,
     DEFAULT_CACHE_SIZE_MB = 256,  //2012.04.24 magic added for 4MB-->256KB
  DEFAULT_PREFETCH_ENABLED = true,
#endif
  SAVE_INTERVAL_SECS = 360
};

char buf_username[256];   // leo added
char buf_password[256];   // leo added
#define dbgmsg(...) \
  do \
    { \
...
tr_sessionGetDefaultSettings (tr_variant * d)
{
  ...
  // 20150831 leo added for get rpc_usernmae and rpc_passwd from FW  {
    /*FILE *fp;
    fp=fopen("/tmp/transmission_get_up","r+");
    memset(buf_username,0,strlen(buf_username));
    memset(buf_password,0,strlen(buf_password));
    if(fp != NULL)
    {
           fgets(buf_username,256,fp);
           if(buf_username[strlen(buf_username)-1]=='\n')
             {
                buf_username[strlen(buf_username)-1]='\0';
             }
           fgets(buf_password,256,fp);
  ...
  tr_variantDictAddBool (d, TR_KEY_download_queue_enabled,           false); //2013.11.28 magic true -> false
  ...
  tr_variantDictAddBool (d, TR_KEY_rpc_authentication_required,     true);   //leo  false->true
  tr_variantDictAddStr  (d, TR_KEY_rpc_bind_address,                "0.0.0.0");
  tr_variantDictAddBool (d, TR_KEY_rpc_enabled,                     false);
  tr_variantDictAddStr  (d, TR_KEY_rpc_password,                    "admin"); // leo changed
  tr_variantDictAddStr  (d, TR_KEY_rpc_username,                     "admin"); //leo  changed
  ...
  tr_variantDictAddInt  (d, TR_KEY_umask,                           000); //2013.11.27 magic
...
tr_sessionGetSettings (tr_session * s, tr_variant * d)
{
  ...
  tr_variantDictAddInt  (d, TR_KEY_peer_limit_global,            s->peerLimit);
   max_peer_tmp = tr_sessionGetPeerLimit( s ); //2011.12.27 magic
  ...
  tr_variantDictAddInt  (d, TR_KEY_speed_limit_up,               tr_sessionGetSpeedLimit_KBps (s, TR_UP));
    up_rate_tmp = tr_sessionGetSpeedLimit_KBps( s, TR_UP ); //2011.12.27 magic
  tr_variantDictAddBool (d, TR_KEY_speed_limit_up_enabled,       tr_sessionIsSpeedLimited (s, TR_UP));
    up_limit_enable = tr_sessionIsSpeedLimited( s, TR_UP );
                ...
                tr_variant * clientSettings)
{
  ...
  session->tcp_socket = TR_BAD_SOCKET;   //added by Yan 2013.11.27 magic
...
tr_sessionInitImpl (void * vdata)
{
  ...
  **/
  ...
  tr_udpInit (session);
   ...
   tr_tcpInit( session );  //added by Yan 2013.11.27 magic
...
sessionCloseImplStart (tr_session * session)
{
  ...
  tr_tcpUninit( session );    //added by Yan 2013.11.27 magic
              ...
              if ((tor = tr_torrentNew (data->ctor, NULL, NULL)))
                {
                  ...
                  //if((misc_seeding_x==1 )&& ((st->percentComplete == (double)1)) && (st->activity==TR_STATUS_STOPPED)){
                  //    tr_torrentStart( tor );
                  //}
