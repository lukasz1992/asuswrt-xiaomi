...
#include "version.h"
#include "web.h"
#include "socket-server.h" //2011.12.20 magic added
...
{
  assert (idle_data == NULL);
  char setcom[256]; //2013.11.27 magic add
  const char * download_dir = NULL;
  ...
  if (tr_variantDictFindInt (args_in, TR_KEY_peer_limit_global, &i)){
      //tr_sessionSetPeerLimit (session, i);
        {
            tr_sessionSetPeerLimit( session, i );
            //2012.01.10 magic added{
            max_peer_tmp = i;
            if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
            memset(setcom,'\0',sizeof(setcom));
                    sprintf(setcom,"sed -i '4s/^.*$/Max_peer=%lld/' /opt/etc/dm2_transmission.conf",i);
                    system(setcom);
            }
            memset(setcom,'\0',sizeof(setcom));
            //sprintf(setcom,"sed -i 's/^.*$/    \"peer-limit-global\": %lld,/' %s/config/settings.json",i,base_path);
            sprintf(setcom,"sed -i 's/^.*peer-limit-global\":.*/    \"peer-limit-global\": %lld, /g' %s/config/settings.json",i,base_path);
                    system(setcom);
            //2012.01.10 magic }
        }
            }
  ...
  {
          tr_sessionSetPeerLimitPerTorrent( session, i );
          //2012.02.21 magic added{
                  max_torrent_peer_tmp = i;
                  if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                  memset(setcom,'\0',sizeof(setcom));
                          sprintf(setcom,"sed -i '3s/^.*$/Max_torrent_peer=%lld/' /opt/etc/dm2_transmission.conf",i);
                          system(setcom);
                  }
                  memset(setcom,'\0',sizeof(setcom));
                  //sprintf(setcom,"sed -i '27s/^.*$/    \"peer-limit-per-torrent\": %lld,/' %s/config/settings.json",i,base_path);
                  sprintf(setcom,"sed -i 's/^.*peer-limit-per-torrent\":.*/    \"peer-limit-per-torrent\": %lld, /g' %s/config/settings.json",i,base_path);
                          system(setcom);
                  //2012.02.21 magic }
      ...
      //tr_sessionSetPexEnabled (session, boolVal);
  {
          ...
          if(boolVal==0){
              if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                  system("sed -i '10s/^.*$/Enable_pex=0/' /opt/etc/dm2_transmission.conf");
              }
              memset(setcom,'\0',sizeof(setcom));
              //sprintf(setcom,"sed -i '33s/^.*$/    \"pex-enabled\": false,/' %s/config/settings.json",base_path);
              sprintf(setcom,"sed -i 's/^.*pex-enabled\":.*/    \"pex-enabled\": false, /g' %s/config/settings.json",base_path);
                  system(setcom);
          }
          else{
              if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                  system("sed -i '10s/^.*$/Enable_pex=1/' /opt/etc/dm2_transmission.conf");
              }
              memset(setcom,'\0',sizeof(setcom));
              //sprintf(setcom,"sed -i '33s/^.*$/    \"pex-enabled\": true,/' %s/config/settings.json",base_path);
              sprintf(setcom,"sed -i 's/^.*pex-enabled\":.*/    \"pex-enabled\": true, /g' %s/config/settings.json",base_path);
                  system(setcom);
          }
          //2012.02.21 magic }
      }
      ...
      //tr_sessionSetDHTEnabled (session, boolVal);
         {
             ...
             if(boolVal==0){
                 if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                     system("sed -i '5s/^.*$/Enable_dht=0/' /opt/etc/dm2_transmission.conf");
                 }
                 memset(setcom,'\0',sizeof(setcom));
                 //sprintf(setcom,"sed -i '14s/^.*$/    \"dht-enabled\": false,/' %s/config/settings.json",base_path);
                     sprintf(setcom,"sed -i 's/^.*dht-enabled\":.*/    \"dht-enabled\": false, /g' %s/config/settings.json",base_path);
                     system(setcom);
                     system(setcom);
             }
             else{
                 if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                     system("sed -i '5s/^.*$/Enable_dht=1/' /opt/etc/dm2_transmission.conf");
                 }
                 memset(setcom,'\0',sizeof(setcom));
                 //sprintf(setcom,"sed -i '14s/^.*$/    \"dht-enabled\": true,/' %s/config/settings.json",base_path);
                     sprintf(setcom,"sed -i 's/^.*dht-enabled\":.*/    \"dht-enabled\": true, /g' %s/config/settings.json",base_path);
                     system(setcom);
             }
             //2012.01.10 magic }
         }
      ...
      //tr_sessionSetPeerPort (session, i);
          {
              ...
              if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
              memset(setcom,'\0',sizeof(setcom));
                      sprintf(setcom,"sed -i '1s/^.*$/Peer_port=%lld/' /opt/etc/dm2_transmission.conf",i);
                      system(setcom);
              }
              memset(setcom,'\0',sizeof(setcom));
              //sprintf(setcom,"sed -i '28s/^.*$/    \"peer-port\": %lld,/' %s/config/settings.json",i,base_path);
              sprintf(setcom,"sed -i 's/^.*peer-port\":.*/    \"peer-port\": %lld, /g' %s/config/settings.json",i,base_path);
                      system(setcom);
              //2012.01.10 magic }
          }
      ...
      //tr_sessionSetSpeedLimit_KBps (session, TR_DOWN, i);
         {
             ...
             if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
             memset(setcom,'\0',sizeof(setcom));
                     sprintf(setcom,"sed -i '7s/^.*$/Down_rate=%lld/' /opt/etc/dm2_transmission.conf",i);
                     system(setcom);
             }
             memset(setcom,'\0',sizeof(setcom));
             //sprintf(setcom,"sed -i '51s/^.*$/    \"speed-limit-down\": %lld,/' %s/config/settings.json",i,base_path);
             sprintf(setcom,"sed -i 's/^.*speed-limit-down\":.*/    \"speed-limit-down\": %lld, /g' %s/config/settings.json",i,base_path);
                     system(setcom);
         }
      ...
      //tr_sessionLimitSpeed (session, TR_DOWN, boolVal);
         {
             ...
             if(boolVal==0){
                 if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                     system("sed -i '6s/^.*$/Down_limit=0/' /opt/etc/dm2_transmission.conf");
                 }
                 memset(setcom,'\0',sizeof(setcom));
                 //sprintf(setcom,"sed -i '52s/^.*$/    \"speed-limit-down-enabled\": false,/' %s/config/settings.json",base_path);
     sprintf(setcom,"sed -i 's/^.*speed-limit-down-enabled\":.*/    \"speed-limit-down-enabled\": false, /g' %s/config/settings.json",base_path);
                     system(setcom);
             }
             else{
                 if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                     system("sed -i '6s/^.*$/Down_limit=1/' /opt/etc/dm2_transmission.conf");
                 }
                 memset(setcom,'\0',sizeof(setcom));
                 //sprintf(setcom,"sed -i '52s/^.*$/    \"speed-limit-down-enabled\": true,/' %s/config/settings.json",base_path);
     sprintf(setcom,"sed -i 's/^.*speed-limit-down-enabled\":.*/    \"speed-limit-down-enabled\": true, /g' %s/config/settings.json",base_path);
                     system(setcom);
             }
             //2012.01.10 magic }
         }
  ...
  if (tr_variantDictFindInt (args_in, TR_KEY_speed_limit_up, &i))
      //tr_sessionSetSpeedLimit_KBps (session, TR_UP, i);
      {
          ...
          if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
          memset(setcom,'\0',sizeof(setcom));
                  sprintf(setcom,"sed -i '9s/^.*$/Up_rate=%lld/' /opt/etc/dm2_transmission.conf",i);
                  system(setcom);
          }
          memset(setcom,'\0',sizeof(setcom));
          //sprintf(setcom,"sed -i '53s/^.*$/    \"speed-limit-up\": %lld,/' %s/config/settings.json",i,base_path);
          sprintf(setcom,"sed -i 's/^.*speed-limit-up\":.*/    \"speed-limit-up\": %lld, /g' %s/config/settings.json",i,base_path);
                  system(setcom);
          //2012.01.10 magic }
      }
      ...
      //tr_sessionLimitSpeed (session, TR_UP, boolVal);
         {
             ...
             if(boolVal==0){
                     up_limit_enable = 0;
                 if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                     system("sed -i '8s/^.*$/Up_limit=0/' /opt/etc/dm2_transmission.conf");
                 }
                 memset(setcom,'\0',sizeof(setcom));
                 //sprintf(setcom,"sed -i '54s/^.*$/    \"speed-limit-up-enabled\": false,/' %s/config/settings.json",base_path);
     sprintf(setcom,"sed -i 's/^.*speed-limit-up-enabled\":.*/    \"speed-limit-up-enabled\": false, /g' %s/config/settings.json",base_path);
                     system(setcom);
             }
             else{
                     up_limit_enable = 1;
                 if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                     system("sed -i '8s/^.*$/Up_limit=1/' /opt/etc/dm2_transmission.conf");
                 }
                 memset(setcom,'\0',sizeof(setcom));
                 //sprintf(setcom,"sed -i '54s/^.*$/    \"speed-limit-up-enabled\": true,/' %s/config/settings.json",base_path);
     sprintf(setcom,"sed -i 's/^.*speed-limit-up-enabled\":.*/    \"speed-limit-up-enabled\": true, /g' %s/config/settings.json",base_path);
                     system(setcom);
             }
             //2012.01.10 magic }
         }
    ...
    }*/
  {
          if( !strcmp( str, "required" ) ){
              ...
              //2012.01.10 magic added{
              if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                  system("sed -i '2s/^.*$/Auth_type=2/' /opt/etc/dm2_transmission.conf");
              }
              memset(setcom,'\0',sizeof(setcom));
              //sprintf(setcom,"sed -i '16s/^.*$/    \"encryption\": 2,/' %s/config/settings.json",base_path);
  sprintf(setcom,"sed -i 's/^.*encryption\":.*/    \"encryption\": 2, /g' %s/config/settings.json",base_path);
              system(setcom);
              //2012.01.10 magic }
          }
              ...
              tr_sessionSetEncryption( session, TR_CLEAR_PREFERRED );
              if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                  system("sed -i '2s/^.*$/Auth_type=0/' /opt/etc/dm2_transmission.conf");
              }
              memset(setcom,'\0',sizeof(setcom));
              //sprintf(setcom,"sed -i '16s/^.*$/    \"encryption\": 0,/' %s/config/settings.json",base_path);
  sprintf(setcom,"sed -i 's/^.*encryption\":.*/    \"encryption\": 0, /g' %s/config/settings.json",base_path);
              system(setcom);
          }
          else{
              tr_sessionSetEncryption( session, TR_ENCRYPTION_PREFERRED );
              if(access("/opt/etc/dm2_transmission.conf", 0) == 0){
                  system("sed -i '2s/^.*$/Auth_type=1/' /opt/etc/dm2_transmission.conf");
              }
              memset(setcom,'\0',sizeof(setcom));
              //sprintf(setcom,"sed -i '16s/^.*$/    \"encryption\": 1,/' %s/config/settings.json",base_path);
  sprintf(setcom,"sed -i 's/^.*encryption\":.*/    \"encryption\": 1, /g' %s/config/settings.json",base_path);
              system(setcom);
          }
  }
   system("cp -rf /opt/etc/dm2_transmission.conf /tmp/APPS/DM2/Config/dm2_transmission.conf");
