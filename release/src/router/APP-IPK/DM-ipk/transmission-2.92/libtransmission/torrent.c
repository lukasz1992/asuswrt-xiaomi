...
#endif
#include <stdio.h>     //20160419 leo added
#include <assert.h>
#include <math.h>
#include <stdarg.h>
...
#include "verify.h"
#include "version.h"
#include "socket-server.h" //20121219 eric
/***
****
***/
...
torrentInit (tr_torrent * tor, const tr_ctor * ctor)
{
  ...
  torrentInitFromInfo (tor);
  if(dldirchangednum != 0)
         tr_ctorSetDownloadDir(ctor,TR_FORCE,dldir);//eric added for change download dir but then add torrent cannot renew log //2013.11.27 magic
   loaded = tr_torrentLoadResume (tor, ~0, ctor);//reset downloaddir debug by eric
  tor->completeness = tr_cpGetStatus (&tor->completion);
...
freeTorrent (tr_torrent * tor)
{
  ...
  tr_metainfoFree (inf);
 //memset (tor, ~0, sizeof (tr_torrent)); //2013.12.02 magic del
  tr_free (tor);
...
tr_torrentStop (tr_torrent * tor)
{
  ...
  tr_torrentlog(tor);//added by Yan 2013.11.27 magic
}
...
removeTorrent (void * vdata)
{
  ...
  tr_sessionLock (session);
  int id = data->tor->stats.id; //2013.11.27 magic
  if (data->deleteFlag)
  ...
  tr_free (data);
  /*remove the log by Yan*/ //2013.11.27 magic

      char trans_log[16], logname[128];
      memset(trans_log, '\0', sizeof(trans_log));
      memset(logname, '\0', sizeof(logname));
      sprintf(trans_log, "transm_%d", id);
      sprintf(logname, "%s.logs/%s", base_path,trans_log);
      if(remove(logname) < 0){
          fprintf(stderr,"remove log error\n");
      }
      else{
          fprintf(stderr,"remove log successfully\n");
      }

      /*remove the tracker_log by Yan*/

      char tracker_log[16], trackername[128];
      memset(tracker_log, '\0', sizeof(tracker_log));
      memset(trackername, '\0', sizeof(trackername));
      sprintf(tracker_log, "tracker_%d", id);
      sprintf(trackername, "%s.logs/%s", base_path,tracker_log);
      if(remove(trackername) < 0){
          fprintf(stderr,"remove trackerlog error\n");
      }
      else{
          fprintf(stderr,"remove trackerlog successfully\n");
      }
  tr_sessionUnlock (session);
}
       ...
       while(!feof(fp))
        {
           memset(generalbuf,'\0',sizeof(generalbuf));
            fscanf(fp,"%[^\n]%*c",generalbuf);
            if(strncmp(generalbuf,"Download_dir=",13) == 0)
           {
                dir = (char *)malloc(strlen(generalbuf)-12);
                memset(dir,'\0',sizeof(dir));
                strcpy(dir,generalbuf+13);
                break;
           }
           else
            {
...
setLocation (void * vdata)
{
  ...
  //2013.11.27 magic{
   //const char * location = data->location;
      char *dir = get_realdldir();
      char *location = NULL;
   if(strncmp(dir,"none",4) == 0)
  ...
  //tr_sys_dir_create (location, TR_SYS_DIR_CREATE_PARENTS, 0777, NULL);
      ...
      char *p;
      p = my_nstrchr('/',location,4);
      strncpy(mountpath,location,strlen(location)-strlen(p));
      ...
      }//diskname maybe renamed by user
      else if((strncmp(serialtmp,"000000",6) != 0) && (strncmp(producttmp,"0000000",7) != 0))
      {
          ...
          location = basedir;
      }//user umount org_disk and mount new disk
     // tr_mkdirp (location, 0777);

      tr_sys_dir_create (location, TR_SYS_DIR_CREATE_PARENTS, 0777, NULL);

    char diskname[100];
    char orgdiskname[100];

    memset(diskname,    '\0', sizeof(diskname));
    memset(orgdiskname, '\0', sizeof(orgdiskname));

    char *folderpoint, *orgfoldpoint;

    folderpoint     = my_nstrchr('/', location,          4);
    orgfoldpoint    = my_nstrchr('/', tor->currentDir,   4);

    strncpy(diskname,    location,          strlen(location)-strlen(folderpoint));
    strncpy(orgdiskname, tor->currentDir,   strlen(tor->currentDir)-strlen(orgfoldpoint));
  ...
  if (!tr_sys_path_is_same (location, tor->currentDir, NULL))
    {
      ...
      /* if the partition of incomplete and complete dir don't same,
       * then tr will check the remaining space of the destination
       * partition */
      if(strcmp(diskname, orgdiskname) != 0 && tor->info.totalSize > tr_getDirFreeSpace(diskname))
      {
        err = TRUE;
        tor->move_failed = TRUE;
      }
            if((tor->have4Gfile == 1) && (disks_tmp != NULL))
            {
                if((strncmp(disks_tmp->disktype,"vfat",4) == 0)||(strncmp(disks_tmp->disktype,"tfat",4) == 0))
                {
                    err = TRUE;
                    tor->isDone = TRUE;
                }
   ...
   tr_torrentlog(tor); //2013.11.27 magic
  if (!err && do_move)
    {
      tr_free (tor->incompleteDir);
  ...
  for (i=0; i<tor->info.fileCount; ++i)
    {
      ...
      //2013.11.27 magic add{
     if( tr_cpGetStatus(&tor->completion)==TR_SEED && misc_seeding_x==0 )//eirc added 2013.5.28
           {
               tor->isStopping = TRUE;
               tor->isFinished = TRUE;
           }
