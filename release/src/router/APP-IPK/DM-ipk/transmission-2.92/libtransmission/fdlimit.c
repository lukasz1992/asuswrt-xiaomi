...
static bool
preallocate_file_sparse (tr_sys_file_t fd, tr_session * session, int id, uint64_t length, tr_error ** error)
{

  tr_error * my_error = NULL;
  tr_torrent * tor_hash = NULL;
      tor_hash = tr_torrentFindFromId( session, id );
      tr_stat *st;
      st = &tor_hash->stats;
  if (length == 0)
    return true;
  ...
  if (!TR_ERROR_IS_ENOSPC (my_error->code))
    {
      ...
      st->hash_flag = 1;
          tr_torrentlog(tor_hash);//write log
  ...
  if (writable && !already_existed && allocation != TR_PREALLOCATE_NONE)
    {
      ...
      if (allocation == TR_PREALLOCATE_FULL)
      ...
      else if (allocation == TR_PREALLOCATE_SPARSE)
        {
          ...
          success = preallocate_file_sparse (fd, session, torrent_id, file_size, &error);
                   ...
                   uint64_t                 file_size)
{
  ...
  if (!cached_file_is_open (o))
    {
      const int err = cached_file_open (o, torrent_id, session, filename, writable, allocation, file_size);
      if (err)
        {
          errno = err;
          return TR_BAD_SYS_FILE;
