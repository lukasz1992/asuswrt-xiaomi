 ...
 */
#include <stdio.h>   //neal add
#include <stdlib.h> //neal add
#include <assert.h>
...
char * get_hostname(char *url)
{
    ...
    if(i>255)
        return NULL;
    check_hostname = (char *)malloc(i+1);
    if(NULL == check_hostname)
        return check_hostname;
    memset(check_hostname,0,sizeof(check_hostname));
    strncpy(check_hostname, url, i);
    check_hostname[i] = '\0';
    return check_hostname;
}
...
int get_port(char *url)
{
    ...
    if(i>8)
        strncpy(check_port, url+1, 7); //neal
    else {
        strncpy(check_port, url+1, i-1); //
        check_port[i] = '\0';
    }
    return atoi(check_port);
}

int  check_tracker_host(char *url){
        struct addrinfo * res, *pt;
        struct sockaddr_in *sinp;
        const char *addr;
        char abuf[INET_ADDRSTRLEN];
        int succ=0,i=0;
        char *check_url, *check_host_name;
                check_url=url;
        if(get_port(check_url)!=80){
                return 0;
            }else{
            check_host_name =get_hostname(check_url);
            }
        //neal add begin
        if(NULL == check_host_name) {
            fprintf(stderr, "Can't get the hostname \n");
            return 1;
        } else {
            succ = getaddrinfo(check_host_name, NULL, NULL, &res);
            free(check_host_name);
        }
        if(succ != 0)
        {
            fprintf(stderr, "Can't get address info! error code = %d", succ);
            return 1;
        }
        FILE *fp;
        char lan_ip_addr[16];
        ...
        memset(lan_ip_tmp,0,sizeof(lan_ip_tmp));
        ///leo {
        if((fp = fopen("/tmp/asus_router.conf", "r")) < 0)
        {
            fprintf(stderr,"\nread asus_router.conf error!\n");
            fprintf(stderr, "tr can not open lan ip addr\n");
            freeaddrinfo(res);
            return 1;
        }
        else
        {
            ...
            while(fgets(lan_ip_tmp, 256, fp)!=NULL)
            {
              if(strstr(lan_ip_tmp,"LAN_IP="))
                {
               snprintf(lan_ip_addr,strlen(lan_ip_tmp+7),"%s",lan_ip_tmp+7);
        ...
        /*if(access("/userfs/bin/tcapi",0) == 0){
            system("/userfs/bin/tcapi get Lan_Entry IP > /tmp/lan_ipaddr");
        }
        else
            system("nvram get lan_ipaddr > /tmp/lan_ipaddr");
        fp = fopen("/tmp/lan_ipaddr", "r");
        if (NULL == fp) {
            fprintf(stderr, "tr can not open lan ip addr\n");
            freeaddrinfo(res);
            return 1;
        } else {
            if(NULL != fgets(lan_ip_addr, 16, fp)) {
                len = strlen(lan_ip_addr);
                lan_ip_addr[len-1] = '\0';
                fclose(fp);
            } else {
                fclose(fp);
                fprintf(stderr, "tr can not get lan ip addr\n");
                freeaddrinfo(res);
                return 1;
            }
        }
        ...
        for(pt=res, i=0; pt != NULL; pt=pt->ai_next, i++){
            ...
            //fprintf(stderr, "addr = %s \t lan ip = %s \n", addr, lan_ip_addr);
                if(!strcmp(addr,"127.0.0.1") || !strcmp(addr,lan_ip_addr)){ //neal change lan_ip_addr
                    freeaddrinfo(res);//neal
                    return 1;
                }
        }
        freeaddrinfo(res);//neal
      ...
      while (web->tasks != NULL)
        {
          ...
          //printf ("adding task to curl: [%s]\n", task->url);
                    if(!strstr(task->url,"ipv6.torrent.ubuntu.com")){
                        if(check_tracker_host(task->url)!=1){
