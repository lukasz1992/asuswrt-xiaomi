...
#include "utils.h"
#include "webseed.h"
#include "socket-server.h" /*added by eric*/ //2013.11.27 magic
...
/* do we still want this piece and does the peer have it? */ //2013.11.27 magic
static bool
...
/*get tor health added by Yan*/ //2013.11.27 magic
void tr_health(tr_torrent * tor)
{
    double tor_health;
    ...
    else if( !tr_torrentIsPieceTransferAllowed( tor, TR_PEER_TO_CLIENT ) ){
        tor_health = -1.0;
        }
    else
    {
        const tr_stat * st = tr_torrentStat((tr_torrent*)tor);
        const tr_info * inf = tr_torrentInfo((tr_torrent*) tor );
        tr_swarm * t = tor->swarm;
        tr_piece_index_t m , n, x, pieceCount = inf->pieceCount, pieceCount_need = inf->pieceCount;

        int i, totalcount_nothave = 0, totalcount_have = 0, peerCount = st->peersConnected, piece[pieceCount];
        ...
        for(i = 0; i < peerCount; i++ )
        {
            ...
            for( m = 0, n = pieceCount; m<n; ++m )
                if( isPieceInteresting( tor, peer, m ) )
                    piece[m]++;
        }
        for (x = 0; x < pieceCount; x++)
        {
            if(!tr_cpPieceIsComplete( &tor->completion, x))  /* we don't have it */
            {
                if(piece[x] == 0)
                    totalcount_nothave++;
                else
                    totalcount_have = totalcount_have + piece[x];
            }
            else                                                /* we have it */
                pieceCount_need--;
        }

        if((totalcount_nothave == 0)&& (totalcount_have != 0))
            tor_health = (double)totalcount_have/pieceCount_need;
        else
            tor_health = (double)(pieceCount_need - totalcount_nothave)/pieceCount_need;
    }

    tor->healthRatio = tor_health;
}
...
rechokePulse (evutil_socket_t foo UNUSED, short bar UNUSED, void * vmgr)
{
  ...
  while ((tor = tr_torrentNext (mgr->session, tor)))
    {
      tr_health(tor); //2013.11.27 magic
