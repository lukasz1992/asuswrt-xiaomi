...
#include <fcntl.h> /* open */     //20160419 added by leo for O_RDONLY
...
#include <libtransmission/socket-server.h> /*added by eric*/
...
static const struct tr_option options[] =
{
    ...
    { 'G', "log-dir", "where to save log", "G", 1, "<path>" },	//Added by Yan
    ...
    while ((c = tr_getopt (getUsage (), argc, argv, options, &optarg))) {
        switch (c) {
            ...
            case 'w': tr_variantDictAddStr (settings, TR_KEY_download_dir, optarg);
                       if(init_diskinfo_struct()==-1)
                                    ...
                                    if(fp)
                                    {
                                        ...
                                        fclose(fp); //2016.8.22 tina add
                                    }
                                    //fclose(fp);//2016.8.22 tina modify
                      break;
            ...
            case 831: tr_variantDictAddBool (settings, TR_KEY_utp_enabled, false);
                      break;
            case 'G': base_path = optarg;   //Add by Yan
                      //printf("base_path: %s \n", base_path);
                      //printf("configDir: %s \n", configDir);
                      break;
...
cleanup:
    ...
    pumpLogMessages (logfile);
    free_disk_struc(&follow_disk_info_start); //2013.11.27 magic
    printf (" done.\n");
         ...
         char * argv[])
{
    ...
    *configDir = getConfigDir (argc, (const char* const *)argv);
    ...
    //2012.01.10 magic added{
        ...
        {
                    char loadcom[256];
                    memset(loadcom,'\0',sizeof(loadcom));
                    sprintf(loadcom,"rm -rf %s/settings.json",configDir);
                    system(loadcom);
                     loaded = tr_sessionLoadSettings( settings, configDir, MY_NAME );
                     ...
                     if(access("/opt/etc/dm2_transmission.conf", 0) == 0)
                    {
                             int fd_bt, len, i=0;
                                                ...
                                                if((fd_bt = open("/opt/etc/dm2_transmission.conf", O_RDONLY | O_NONBLOCK)) < 0)
                                                {
                                                    fprintf(stderr,"\nread conf error!\n");
                                                }
                                                else
                                                {
                                                        ...
                                                        while((len = read(fd_bt, &ch, 1)) > 0)
                                                        {
                                                            ...
                                                            else if(ch == '\n')
                                                            {
                                                                ...
                                                                {
                                                                                    tr_variantDictAddInt( settings, TR_KEY_peer_port, atoi( content ) );

                                                                }
                                                        else if(strcmp(name, "Auth_type") == 0)
                                                        {
                                                                                    if(atoi(content)==0){
                                                                                            tr_variantDictAddInt( settings, TR_KEY_encryption,  TR_CLEAR_PREFERRED );
                                                                                    }
                                                                                    else if(atoi(content)==1){
                                                        ...
                                                        else if(!strcmp(name, "Max_torrent_peer"))
                                                                {
                                                            tr_variantDictAddInt( settings, TR_KEY_peer_limit_per_torrent, atoi( content ) );
                                                                }
                                                        else if(!strcmp(name, "Max_peer"))
                                                                {
                                                            tr_variantDictAddInt( settings, TR_KEY_peer_limit_global, atoi( content ) );
                                                                }
                                                        else if(!strcmp(name, "Enable_dht"))
                                                                {
                                                            if(atoi(content)==0){
                                                        ...
                                                        else if(!strcmp(name, "Down_limit"))
                                                                {
                                                            if(atoi(content)==0){
                                                                            tr_variantDictAddBool( settings, TR_KEY_speed_limit_down_enabled,  FALSE );
                                                            }
                                                            else{
                                                                            tr_variantDictAddBool( settings, TR_KEY_speed_limit_down_enabled,  TRUE );
                                                            }
                                                                }
                                                        else if(!strcmp(name, "Down_rate"))
                                                                {
                                                            tr_variantDictAddInt ( settings, TR_KEY_speed_limit_down, atoi( content ) );
                                                                }
                                                        else if(!strcmp(name, "Up_limit"))
                                                                {
                                                            if(atoi(content)==0){
                                                                            tr_variantDictAddBool( settings, TR_KEY_speed_limit_up_enabled,  FALSE );
                                                            }
                                                            else{
                                                                                                    tr_variantDictAddBool( settings, TR_KEY_speed_limit_up_enabled,  TRUE );
                                                            }
                                                                }
                                                        else if(!strcmp(name, "Up_rate"))
                                                                {
                                                            tr_variantDictAddInt ( settings, TR_KEY_speed_limit_up, atoi( content ) );
                                                                }
                                                        else if(!strcmp(name, "Enable_pex"))
                                                                {
                                                            if(atoi(content)==0){
                                                                    tr_variantDictAddBool( settings, TR_KEY_pex_enabled, FALSE );
                                                            }
                                                            else{
                                                                    tr_variantDictAddBool( settings, TR_KEY_pex_enabled, TRUE );
                                                            }
                                                                }
                                                                continue;
                                                            }
