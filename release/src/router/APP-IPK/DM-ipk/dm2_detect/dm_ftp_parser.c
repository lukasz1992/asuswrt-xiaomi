...
char *
get_line(UrlResource *rsrc, int control ,int times)
{
    int bytes_read	= 0;
    char *end;
    char buf[BUFSIZE+1];
...
int
get_passive_sock(UrlResource *rsrc, int control ,int times)
{
    unsigned char *addr;
    ...
    if( !((line = get_line(rsrc, control ,times)) &&
...
int
get_sock(UrlResource *rsrc, int control ,int times)
{
    struct sockaddr_in sa;
    unsigned char *addr;
    unsigned char *port;
...
int
ftp_transfer(UrlResource *rsrc ,int times)
{
    ...
    if( !(line = get_line(rsrc, sock ,times)) )
    {
        //return 0;
        rsrc->url->status = P_FAIL;
        return  P_FAIL;
