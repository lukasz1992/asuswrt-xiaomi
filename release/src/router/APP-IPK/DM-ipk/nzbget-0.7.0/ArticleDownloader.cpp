...
ArticleDownloader::EStatus ArticleDownloader::Download()
{
	...
	Status = adRunning;
        int temp_time;   // add by gauss for only update log one time per second
...
#if 1
                if(!force_pause_nzb && !auto_paused)
                {
                    ...
                    //info("download time second is %d",iDownloadTimeSec);

                    if(iDownloadTimeSec % 5 == 0)
                    {


                        if(temp_time != iDownloadTimeSec)
                        {
                            ...
                            for (GroupQueue::iterator it = groupQueue.begin(); it != groupQueue.end(); it++)
                            {
                                ...
                                unsigned long long iRemainingSize;
                                int ID;
                                ...
                                iDownloadRate = (int)(g_pQueueCoordinator->CalcCurrentDownloadSpeed());
                                percent_done = (float) (iFileSize -iRemainingSize ) / iFileSize;
                                ...
                                char mountpath[100];
                                    ...
                                    char *p;
                                    p = disksave.my_nstrchr('/',info_dest_path,4);
                                    strncpy(mountpath,info_dest_path,strlen(info_dest_path)-strlen(p));
                                ...
                                if(fp)
                                {
                                    int read_lenth = fread(&log_s,  LOG_SIZE, 1,fp);
                                    rewind(fp);
                                    ...
                                    if(!log_s.begin_t)   //first time fill value
                                    {
                                    time(&log_s.begin_t);
                                    time(&log_s.now_t);
                                   ...
                                   // if(strlen(log_s.ifile.inzb.Destination) == 0) //first time fill value
                                    //{
                                        //char info_dest_path[256];
                                        //memset(info_dest_path,'\0',sizeof(info_dest_path));
                                        ...
                                        log_s.ifile.inzb.partitionnum = 0;
                                        //if(disks_tmp!=NULL){
                                        // info("magic1 disks_tmp->serialnum=%s",disks_tmp->serialnum);
                                        //     info("magic1 disks_tmp->partitionport=%d",disks_tmp->partitionport);
                                        //}
                                        if((disks_tmp!=NULL)&&(disks_tmp->partitionport != 0))
                                         {
                                            //info("magic disks_tmp->serialnum=%s",disks_tmp->serialnum);
                                             sprintf(log_s.ifile.inzb.serial, "%s",disks_tmp->serialnum);
                                             sprintf(log_s.ifile.inzb.pid, "%s",disks_tmp->product);
                                             sprintf(log_s.ifile.inzb.vid, "%s",disks_tmp->vendor);
                                             log_s.ifile.inzb.partitionnum = disks_tmp->partitionport;
                                         }
                                         else
                                         {
                                             memset(log_s.ifile.inzb.serial,0,sizeof(log_s.ifile.inzb.serial));
                                             memset(log_s.ifile.inzb.pid,0,sizeof(log_s.ifile.inzb.pid));
                                             memset(log_s.ifile.inzb.vid,0,sizeof(log_s.ifile.inzb.vid));
                                         }

                                         //info("magic log_s.ifile.inzb.serial=%s",log_s.ifile.inzb.serial);
                                         //info("magic log_s.ifile.inzb.pid=%s",log_s.ifile.inzb.pid);
                                       //disksave.free_disk_struc(&(disksave.follow_disk_info_start));
                                        char *realdldir;
                                          realdldir = disksave.my_nstrchr('/',info_dest_path,4);
                                          if ( !access(info_dest_path,0) ){
                                                memset(log_s.ifile.inzb.Destination,'\0',sizeof(log_s.ifile.inzb.Destination));
                                                 sprintf(log_s.ifile.inzb.Destination, "%s",info_dest_path);
                                          }
                                          else{
                                                 memset(log_s.ifile.inzb.Destination,'\0',sizeof(log_s.ifile.inzb.Destination));
                                                  sprintf(log_s.ifile.inzb.Destination, "%s%s","not_found",realdldir);
                                              }
