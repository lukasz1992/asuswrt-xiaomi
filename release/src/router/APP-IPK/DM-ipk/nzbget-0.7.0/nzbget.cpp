...
void DisableCout();
#endif
int GetInit4GBErrorCount();
bool CheckMountPathExist();
...
int force_pause_nzb = 0;
int have_error_log = 0;
int force_cancle_nzb = 0;
int error_time = 0;
int have_update_log = 0 ;
int not_complete = 0;
int auto_paused = 0;
int above_4GB = 0; //2013/9/17 add by gauss
int iInit4GBErrorCount = 0;
char szRecord4GBFile[256];
bool bDelInit = false;
//bool bDelQueueEnd = false;
bool bParseSeedEnd = false;
...
int main(int argc, char *argv[], char *argp[])
{
	...
	DisableCout();
#endif
        /*check mount path is exist add by gauss 2013/11/18*/
        if(!CheckMountPathExist())
        {
           printf("mount path is not exist,nzb exit\n");
           return -1;
        }
        ...
        if(fp)
        {
            while(!feof(fp))
            {
                fscanf(fp,"%[^\n]%*c",disksave.generalbuf);
                if(strncmp(disksave.generalbuf,"serial=",7) == 0)
                {
                    strcpy(disksave.serialtmp,disksave.generalbuf+7);
                }
                else if(strncmp(disksave.generalbuf,"vonder=",7) == 0)
                {
                    strcpy(disksave.vondertmp,disksave.generalbuf+7);
                }
                else if(strncmp(disksave.generalbuf,"product=",8) == 0)
                {
                    strcpy(disksave.producttmp,disksave.generalbuf+8);
                }
                else if(strncmp(disksave.generalbuf,"partition=",10) == 0)
                {
                    disksave.partitiontmp = atoi(disksave.generalbuf+10);
                }
            }
	...
	g_pOptions = new Options(argc, argv);
        //     g_pOptions->SetDestDir("/home/walf/"); //Walf: Set dest dir
...
#endif
#endif
        iInit4GBErrorCount =  GetInit4GBErrorCount();
...
{
    char szPath[256] = {0};
    int iLen = strlen(g_pOptions->GetNzbDir());
    strncpy(szPath,g_pOptions->GetNzbDir(),iLen-4); //trim tail '/nzb'
    sprintf(szRecord4GBFile,"%s/NZB4GBFailCount",szPath);
    if(access(szRecord4GBFile,0) == 0)
    {
        FILE *fp = fopen(szRecord4GBFile,"r");
        if(fp == NULL)
        {
            info("open %s fail",szRecord4GBFile);
            return 0;
        }
        char szBuf[4] = {0};
        fread(szBuf,4,1,fp);
...
bool CheckMountPathExist()
{
    ...
    if(fp)
    {
        ...
        {
            fscanf(fp,"%[^\n]%*c",generalbuf);
            if(strncmp(generalbuf,"$EX_MAINDIR=",11) == 0)
            {
                strncpy(mount_path,generalbuf+12,1024);
