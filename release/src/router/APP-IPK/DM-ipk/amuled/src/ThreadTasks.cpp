...
extern char dm2_basedir[128];
extern char dldir[1024];
...
extern char *my_nstrchr(const char chr,char *str,int n);
...
void CCompletionTask::Entry()
{
	CPath targetPath;
        ...
        char basedir[strlen(dm2_basedir)+20];
        memset(basedir,'\0',sizeof(basedir));
        strncpy(basedir,dm2_basedir,strlen(dm2_basedir));
        /*********************eric added for diskinfo*****************/
	...
	CPath partfilename = m_metPath.RemoveExt();
        ...
        /*********************eric added for diskinfo*****************/
        wxString filename_eric;
        wxString fullname_eric;
        char filename[1024];
        memset(filename,'\0',sizeof(filename));
        char fullname[1024];
        memset(fullname,'\0',sizeof(fullname));
        filename_eric = CFormat(_("%s")) % newName.GetPath();
        fullname_eric = CFormat(_("%s")) % newName.GetFullName();
        strcpy(filename,filename_eric.mb_str(wxConvUTF8));
        strcpy(fullname,fullname_eric.mb_str(wxConvUTF8));
        ...
        //sprintf(filefullname,"%s%s%s",filename,"/",fullname);

        char mountpath[100];
        memset(mountpath,'\0',sizeof(mountpath));
        char *p;
        p = my_nstrchr('/',dldir,4);
        strncpy(mountpath,dldir,strlen(dldir)-strlen(p));
        ...
        if(disks_tmp)
        {
            ...
            if(strncmp(mountpath,disks_tmp->mountpath,strlen(disks_tmp->mountpath)) != 0)
            {
                ...
                strcat(realpath,p);
                memset(filename,'\0',sizeof(filename));
                strncpy(filename,realpath,strlen(realpath));
                    /*if(access(filename, F_OK) != 0) {
                	//fprintf(stderr,"\nfilename11::%s not exist\n",filename);
                	mkdir(filename,S_IRWXU | S_IRWXG | S_IRWXO);//eric added
                    }*/
                dm2_mkdirp(filename, 0777);
                sprintf(filefullname,"%s%s%s",filename,"/",fullname);
                strcat(filename,"/");
                strcat(filename,"asustmp_file");
                wxString dldirtmp(filename,wxConvUTF8);
                newName.dirset(dldirtmp);
            }
            else
            {
                memset(filename,'\0',sizeof(filename));
                strncpy(filename,dldir,strlen(dldir));
            /*if(access(filename, F_OK) != 0) {
                //fprintf(stderr,"\nfilename22::%s not exist\n",filename);
                mkdir(filename,S_IRWXU | S_IRWXG | S_IRWXO);//eric added
            }*/
                dm2_mkdirp(filename, 0777);
                sprintf(filefullname,"%s%s%s",filename,"/",fullname);
                strcat(filename,"/");
                strcat(filename,"asustmp_file");
                wxString dldirtmp = wxString(filename,wxConvUTF8);
                newName.dirset(dldirtmp);
            }
        }
        else
        {
            strcat(basedir,"/Download2/Complete");
            memset(filename,'\0',sizeof(filename));
	...
	if (!CPath::RenameFile(partfilename, newName)) {
        ...
        {
            if(strstr(filename,fullname) != NULL)
            {
                fprintf(stderr,"\nno need to rename file\n");
            }
            else
            {
                if(rename(filename,filefullname) != 0)
                {
                    m_error = true;
                    return;
                }
