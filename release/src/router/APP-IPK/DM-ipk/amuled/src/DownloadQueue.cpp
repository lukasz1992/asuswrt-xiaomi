...
#include "log.h" //2011.10 magic added
...
extern char dm2_basedir[128]; //2012.02 magic added
...
char* CDownloadQueue::getbasepath()
{
...
void CDownloadQueue::init_log(CPartFile* file, uint16 id){ //2011.10 magic added
	int fd ,n;
	Log_struc  log_p;
	char logname[512] ,fullname_new[16];
	wxString filename, hash ,fullname;
	memset(fullname_new, '\0', sizeof(fullname_new));
	fullname = CFormat(_("%s")) % file->GetPartMetFileName();
	strncpy(fullname_new,fullname.mb_str(),3);
	memset(logname, '\0', sizeof(logname));

	sprintf(logname, "%s/Download2/.logs/ed2k_%s", dm2_basedir, fullname_new);

	struct disk_info *disk_tmp;
	disk_tmp = disksave.follow_disk_info_start;
	while(disk_tmp != NULL)
	{
		if(disk_tmp->diskname != NULL)
		{
			if((strncmp(disksave.serialtmp,disk_tmp->serialnum,strlen(disk_tmp->serialnum)) == 0)&&(strncmp(disksave.producttmp,disk_tmp->product,strlen(disk_tmp->product)) == 0)&&(strncmp(disksave.vondertmp,disk_tmp->vendor,strlen(disk_tmp->vendor)) == 0)&&(disksave.partitiontmp==disk_tmp->partitionport))
			{
				break;
			}
			else
	...
	if(strcmp(fullname_new,"")!=0){
		...
		{
			int peers = 0;
			char filename_new[1024],hash_new[64];
			double downspeed,uploadSpeed;

			log_p.ifile.iEd2k.partitionnum = 0;
			if((disk_tmp!=NULL)&&(disk_tmp->partitionport != 0))
			{
				sprintf(log_p.ifile.iEd2k.serial, "%s",disk_tmp->serialnum);
				sprintf(log_p.ifile.iEd2k.pid, "%s",disk_tmp->product);
				sprintf(log_p.ifile.iEd2k.vid, "%s",disk_tmp->vendor);
				log_p.ifile.iEd2k.partitionnum = disk_tmp->partitionport;
			}
			else
			{
				memset(log_p.ifile.iEd2k.serial,0,sizeof(log_p.ifile.iEd2k.serial));
				memset(log_p.ifile.iEd2k.pid,0,sizeof(log_p.ifile.iEd2k.pid));
				memset(log_p.ifile.iEd2k.vid,0,sizeof(log_p.ifile.iEd2k.vid));
			}

			wxString filepath;
			char filepath_new[64];
			memset(filepath_new, '\0', sizeof(filepath_new));
			filepath = CFormat(wxT("%s")) % thePrefs::GetIncomingDir();
			strcpy(filepath_new,filepath.mb_str(wxConvUTF8));

			if((disk_tmp)&&(strncmp(filepath_new,disk_tmp->mountpath,strlen(disk_tmp->mountpath)) != 0))
			{
				char *p;
				p = my_nstrchr('/',filepath_new,4);
				if(p){
					char downloaddirtmp[strlen(p)+1];
					memset(downloaddirtmp,'\0',sizeof(downloaddirtmp));
					strncpy(downloaddirtmp,p,strlen(p));
					memset(filepath_new,'\0',sizeof(filepath_new));
					strcpy(filepath_new,disk_tmp->mountpath);
					strcat(filepath_new,downloaddirtmp);}
				else
				{
					memset(filepath_new,'\0',sizeof(filepath_new));
					strcpy(filepath_new,disk_tmp->mountpath);
			...
			char *realdldir;
			realdldir = my_nstrchr('/',filepath_new,4);
			memset(log_p.ifile.iEd2k.downloadDir,'\0',sizeof(log_p.ifile.iEd2k.downloadDir));
			if(!access(filepath_new,0))
				sprintf(log_p.ifile.iEd2k.downloadDir,"%s",filepath_new);
			else
				sprintf(log_p.ifile.iEd2k.downloadDir,"%s%s","not_found",realdldir);

			memset(filename_new, '\0', sizeof(filename_new));
			memset(hash_new, '\0', sizeof(hash_new));

			filename = CFormat(wxT("%s")) % file->GetFileName();
			strcpy(filename_new,filename.mb_str(wxConvUTF8));
			sprintf(log_p.filename,"%s",filename_new);

			hash = CFormat(wxT("%s")) % file->GetFileHash().Encode();
			strcpy(hash_new,hash.mb_str());
			sprintf(log_p.id,"%s",hash_new);

			log_p.download_type = ED2K;

			uint8 status = file->GetStatus();
			if(status == PS_PAUSED ) {
				log_p.progress = (float)(file->GetPercentCompleted()/100);
				log_p.status = S_PAUSED;
			} else if(status == PS_COMPLETE) {
				log_p.progress = 1;
				log_p.status = S_COMPLETED;
			} else if(status == PS_INSUFFICIENT) {
				log_p.progress = (float)(file->GetPercentCompleted()/100);
				log_p.status = S_DISKFULL;
			} else if(status == PS_ERROR) {
				log_p.progress = 0;
				log_p.status = S_DEAD_OF_ERROR;
			} else {
				if(((int)file->GetPercentCompleted()) == 100)
					log_p.progress = 0.9999;
				else
					log_p.progress = (float)(file->GetPercentCompleted()/100);
			...
			log_p.filesize = file->GetFileSize();

			downspeed = (double)file->GetKBpsDown();
			//uploadSpeed = (double)theStats::GetUploadRate();
			uploadSpeed = 0;


			log_p.ifile.iEd2k.downloadSpeed=downspeed;
			log_p.ifile.iEd2k.uploadSpeed=uploadSpeed;

			peers = (int)(file->GetSourceCount()-file->GetNotCurrentSourcesCount());
			if (peers < 0)
				log_p.ifile.iEd2k.downloadpeers = 0;
			else
				log_p.ifile.iEd2k.downloadpeers = peers;

			if(write(fd, &log_p, sizeof(Log_struc)) < 0)
...
{
	//fprintf(stderr,"\n CDownloadQueue::CheckAndAddSource\n");
...
void CDownloadQueue::RemoveFile(CPartFile* file, bool keepAsCompleted)
{
	...
	wxString fullname;
	char command[128],fullname_new[16];
    ...
    if(NULL == basepath) {
		...
		snprintf(command, sizeof(command), \
				 "rm -rf \"%s/Download2/.logs/ed2k_%s\"", basepath, fullname_new);
        free(basepath);
        basepath == NULL;
    }
	system(command);
...
bool CDownloadQueue::AddED2KLink( const CED2KServerLink* link )
{
    ...
    if(theApp->serverlist->AddServer(server)) {
