...
void CPartFile::CreatePartFile()
{
	...
	do { 
		...
		log_name = CPath(CFormat(wxT("../.logs/ed2k_%03i")) % i);
		m_fullname = thePrefs::GetTempDir().JoinPaths(m_partmetfilename);
		log_path = thePrefs::GetTempDir().JoinPaths(log_name);
	} while (m_fullname.FileExists() || log_path.FileExists());
...
void CPartFile::PauseFile(bool bInsufficient)
{
	...
	//theApp->downloadqueue->init_log(this,getbasepath(),1); //2011.10 magic added
	theApp->downloadqueue->init_log(this,1); //2012.02 magic added
}
