...
void CServerList::ServerStats()
{
	...
	if (theApp->IsConnectedED2K() && m_servers.size() > 0) {
		...
		if (ping_server->GetFailedCount() >= thePrefs::GetDeadserverRetries() && thePrefs::DeadServer() && !ping_server->IsStaticMember()) {
            //add for note the server removed by amule
            FILE *fp;
            if(fp = fopen("/tmp/APPS/DM2/Config/dm2_amule/server_removed", "a+"))
            {
                char ip[16];
                char tmp[50];
                int port = 0;
                ...
                strncpy(ip, (ping_server->GetFullIP()).mb_str(wxConvUTF8),15);
                port = ping_server->GetPort();
                snprintf(tmp, sizeof(tmp),"ip:%s\nport:%d\n", ip, port);
                fprintf(stderr, "tmp %s\n", tmp);
                fwrite(tmp, strlen(tmp), 1, fp);
                fclose(fp);
            }
            system("cp /tmp/APPS/DM2/Config/dm2_amule/server_removed /opt/etc/dm2_amule/server_removed");
...
bool CServerList::SaveServerMet()
{
    FILE *fp;
    char tmp[32];
    fp = fopen("/tmp/APPS/DM2/Config/dm2_amule/aMule_server", "w+");
	...
	try {
		...
		for ( CInternalList::const_iterator it = m_servers.begin(); it != m_servers.end(); ++it) {
            ...
            if(fp) {
                memset(tmp, '\0', sizeof(tmp));
                strncpy(tmp,(server->GetFullIP()).mb_str(wxConvUTF8),16);
    ...
    system("cp -rf /tmp/APPS/DM2/Config/dm2_amule/server.met /opt/etc/dm2_amule/server.met"); //neal
    system("cp -rf /tmp/APPS/DM2/Config/dm2_amule/aMule_server /opt/etc/dm2_amule/aMule_server");
	return true;
}
		...
		for ( CInternalList::const_iterator it = m_servers.begin(); it != m_servers.end(); ) {
			...
			if ( server->GetFailedCount() > thePrefs::GetDeadserverRetries() && !server->IsStaticMember()) {
                ...
                if(fp = fopen("/tmp/APPS/DM2/Config/dm2_amule/server_removed", "a+"))
                {
                    ...
                    memset(ip, '\0', sizeof(ip));
                    memset(tmp, '\0', sizeof(tmp));
                    strncpy(ip, (server->GetFullIP()).mb_str(wxConvUTF8),15);
                    port = server->GetPort();
                    snprintf(tmp, sizeof(tmp),"ip:%s\nport:%d\n", ip, port);
                    fwrite(tmp, strlen(tmp), 1, fp);
                    fclose(fp);
