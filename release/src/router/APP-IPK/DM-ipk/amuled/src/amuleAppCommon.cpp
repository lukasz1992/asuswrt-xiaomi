...
CamuleAppCommon::CamuleAppCommon()
{
	...
	if (IsRemoteGui()) {
		...
		m_configFile	= wxT("dm2_amule.conf");
...
bool CamuleAppCommon::InitCommon(int argc, wxChar ** argv)
{
	...
	//cmdline.AddOption(wxT("b"), wxT("Base_dir"), wxT("set Base dir")); //2011.10 magic added
#ifdef AMULE_DAEMON
...
// Keep stdin open to run valgrind --gen_suppressions
	...
	//if ( cmdline.Found(wxT("Base_dir"), &Base_dir) ) { //2011.10 magic added
	//	wxFileName fn(Base_dir);
	//	fn.MakeAbsolute();
	//	Base_dir = fn.GetFullPath();
	//	AddLogLineNS(CFormat(wxT("(base dir: %s)")) % Base_dir);
		//sprintf(base_dir_path, "%s",Base_dir);
	//}
