	...
	while ( it != connectionattemps.end() ){
		...
		if ( dwCurTick - it->first > CONSERVTIMEOUT) {
			...
			if (!value->IsSolving()) {
                ...
                strncpy(serverip,(value->cur_server->GetFullIP()).mb_str(wxConvUTF8),16);
            if((fd = open("/tmp/dm2_amule_timedout",O_RDWR|O_CREAT|O_TRUNC, 0777))<0){
            } else {
                char tmp_message[100];
                    snprintf(tmp_message, 100, "status:timedout\nserver_ip:%s\n", serverip);
